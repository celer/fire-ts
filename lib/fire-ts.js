...
Fire.process=function(res){
	...
	for(var i in res){
		...
		if(input.string && input.string!=''){
			...
			if(i>0){
				var last = res[i-1];
				if(input.e=='=' || input.e=='%' || input.e=='#'){
					if(last.code || (last.string && (/\n$/.test(last.string) || last.string=='')) || last.nl){
						ret.push({ string:Fire.ts(opts.tabDepth), line: input.line, td: opts.tabDepth });		
					}
...
Fire.parseSync=function(file,opts){
	...
	if(debug) console.log("PARSE",res);
	res = Fire.process(res);
	if(debug) console.log("PROCESS",res);
	res = Fire.optimize(res);
	if(debug) console.log("OPTIMIZE",res);
	res = Fire.toString(res,{async:false});
	if(uglify){
		res=UglifyJS.minify(res,{fromString:true}).code;
	}
	if(debug) console.log("CODE",res);
	return res;
}
		...
		if(input.string){
			...
			if(input.e){
				if(input.e=="="){
					...
					s+=tc+"<FTSP>e(<FTSP>r("+JSON.stringify(input.code.trim())+",<FTSP>i,<FTSP>o));\n";
				} else if(input.e=='%'){			
